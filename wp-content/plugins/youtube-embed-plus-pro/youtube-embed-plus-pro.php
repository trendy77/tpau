...
class YouTubePrefsPro
{
    ...
    public static function custom_admin_pointers()
    {
        ...
        {
            $new_pointer_content .= __("This update has code refactoring to separate and streamline the Free and Pro versions.");
        }
...
//            $new_options[self::$opt_hl] = $_hl;
        ...
        {
            wp_enqueue_style('embedplusyoutube', plugins_url() . '/youtube-embed-plus-pro/scripts/embedplus_mce.css');
            ...
            if (
            //(!(isset(self::$alloptions[self::$opt_pro]) && strlen(trim(self::$alloptions[self::$opt_pro])) > 0)) && // display only if not pro ooopointer
                    (get_bloginfo('version') >= '3.3') && self::custom_admin_pointers_check()
            )
            {
