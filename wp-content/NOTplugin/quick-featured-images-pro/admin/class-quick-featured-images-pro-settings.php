...
class Quick_Featured_Images_Pro_Settings {
	...
	public function get_page_headline() {
		$text = 'Settings';
		//return __( 'Image Columns', 'quick-featured-images-pro' );
		return __( $text );
	}
	...
	public function get_page_description() {
		return __( 'Set the visibility of columns of featured images in posts lists', 'quick-featured-images-pro' );
	}
	...
	private static function set_default_settings() {
		...
		if ( false === get_option( 'quick-featured-images-pro-settings', false ) ) {
			// store default values in the db as a single and serialized entry
			add_option( 
				'quick-featured-images-pro-settings', 
				array(
					'column_thumb_post' => '1',
					'column_thumb_page' => '1',
	...
	*/
	public function register_options () {
		...
		add_settings_section(
			...
			__( 'Columns for featured images in posts lists', 'quick-featured-images-pro' ),
		...
		$title = __( 'Show additional column for featured images in lists of', 'quick-featured-images-pro' );
		add_settings_field(
			// form field name for use in the 'id' attribute of tags
			'column_toggles',
			// title of the form field
			$title . sprintf( '<br />&nbsp;<br /><img src="%s" alt="%s" width="200" height="104" />', plugins_url( 'assets/images/posts_list_w_image_column.gif' , __FILE__ ), __( 'Posts list with image column', 'quick-featured-images-pro' ) ),
		...
		add_settings_section(
			...
			$section_key, 
			// title of the section.
			__( 'Visibility of the plugin', 'quick-featured-images-pro' ),
			// callback function that fills the section with the desired content
			array( $this, 'print_section_' . $section_key ),
			// menu page on which to display this section
			$this->main_options_page_slug
		...
		// register the options for the section
		$title = __( 'Which user role may see the plugin', 'quick-featured-images-pro' );
		add_settings_field(
			// form field name for use in the 'id' attribute of tags
			'allowed_roles',
			// title of the form field
	...
	*/
	public function print_columns_options ( $args ) {
		...
		$html .= '</fieldset>';
		$html .= sprintf( '<p class="description">%s</p>', __( 'Activate the checkboxes at each post type to show the extra columns in the post lists.', 'quick-featured-images-pro' ) );
		print $html;
	}
	...
	*/
	public function print_role_control ( $args ) {
		// get translations
		$label = __( 'Minimum user role to see the plugin in the backend', 'quick-featured-images-pro');
		...
		$html .= '</div>';
		$html .= '</fieldset>';
		$html .= sprintf( '<p class="description">%s</p>', __( 'Select the minimum user role a user must have to see the plugin.', 'quick-featured-images-pro' ) );
		$html .= sprintf( '<p class="description">%s</p>', sprintf( __( 'The rules as set in &#8220;%s&#8221; works on posts independently of this setting.', 'quick-featured-images-pro' ), __( 'Preset Featured Images', 'quick-featured-images-pro' ) ) );
		$html .= sprintf( '<p class="description">%s</p>', __( 'This setting controls as well whether a user will see in an image column the thumbnails with action links or the thumbnails only. To switch image columns on and off use the section above.', 'quick-featured-images-pro' ) );
		$html .= sprintf( '<p class="description">%s</p>', sprintf( __( "This page and the page &#8220;%s&#8221; are accessible for administrators only.", 'quick-featured-images-pro' ), __( 'License Activation', 'quick-featured-images-pro' ) ) );
		print $html;
	}
	...
	*/
	public function print_section_1st_section () {
		printf( "<div class=\"qfi_page_description\"><p>%s</p></div>\n", __( 'The additional columns give you a quick overview about all used featured images for every post. The Featured Image column is sortable.', 'quick-featured-images-pro' ) );
	}
	...
	*/
	public function print_section_2nd_section () {
		printf( "<div class=\"qfi_page_description\"><p>%s</p></div>\n", __( 'Controls which minimum user role can see the plugin.', 'quick-featured-images-pro' ) );
	}
